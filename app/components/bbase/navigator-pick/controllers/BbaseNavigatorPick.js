...
/**
 * @description 模块功能说明
 * @class BbaseNavigatorPick
 * @author yongjin<zjut_wyj@163.com> 2016/2/6
 */
define('BbaseNavigatorPick', [], function(require, exports, module) {
  var BbaseNavigatorPick;

  var curList = [];
  var navigatorList = [];
  var tempList = [];

  BbaseNavigatorPick = BbaseList.extend({
    initialize: function() {
      var navigatorIdPath = this.navigatorIdPath = this.options.navigatorIdPath || 'id';
      ...
      this._super({
        template: `
          <div class="BbaseNavigatorPick-wrap bbase-component-navigator-pick">
            <div class="theme-black" style="height: ${height}px;overflow:auto;">
              <div class="form" style="background-color: #fff;padding: 10px;">
                <div id="navigator-widget" class="jhw-product-navigator jhw-navigator-product">
                  <ul class="tree-list-ul navigator-ul">
                  </ul>
                </div>
              </div>
            </div>
        ...
        model: BbaseModel.extend({
          baseId: navigatorIdPath,
          baseUrl: CONST.API + '/product/detail',
          fields: [navigatorIdPath, namePath, imagePath]
        }),
        ...
        item: BbaseItem.extend({
          ...
          template: `
            <div class="grid-td-tree">
              ...
              <!---->
              <!---->
              <div class="cate-grid grid-td-cate-name jhw-navigator navigator-div-{{level}}">
                <div class="cate-grid-cell-inner">
          ...
          init: function() {
            ...
            var _this = this;
            if (BbaseEst.indexOf(curList, _this._get(navigatorIdPath) +'') > -1) {
              _this._set('checked', true);
              var dx = BbaseEst.findIndex(navigatorList, function(item){
                return (item[navigatorIdPath] + '')=== (_this._get(navigatorIdPath) + '');
              });
              if (dx === -1) {
                navigatorList.push(_this.model.toJSON(true));
              }
            } else {
          ...
          add() {
            ...
            if (_this._get('checked')) {
              _this._set('checked', false);
              curList.splice(BbaseEst.indexOf(curList, _this._get(navigatorIdPath) +''), 1);
               var dx = BbaseEst.findIndex(navigatorList, function(item){
                return (item[navigatorIdPath] +'') === (_this._get(navigatorIdPath) + '');
              });
              navigatorList.splice(dx, 1);
            } else {
              _this._set('checked', true);
              if (BbaseEst.indexOf(curList, _this._get(navigatorIdPath) + '') === -1) {
                curList.push(_this._get(navigatorIdPath) + '');
                navigatorList.push(_this.model.toJSON(true));
              }
            }
          },
        ...
        navigatorId: this.navigatorIdPath,
    ...
    beforeRender() {
      ...
      if (BbaseEst.isEmpty(this._get('cur'))) {
        navigatorList = this._get('navigatorList');
        curList = BbaseEst.pluck(navigatorList, this.navigatorIdPath);
      } else {
    ...
    save() {
      if (this._options.onChange) {
        this._options.onChange.call(this, curList.join(',').replace(/,,/img, '').replace(/^,/img, ''), navigatorList);
      }
      this._close();
    }
  });

  module.exports = BbaseNavigatorPick;
});