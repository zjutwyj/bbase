...
QUnit.test("each -> 1ms *1", function(assert) {
  ...
  BbaseEst.each(list1, function(item) {
...
QUnit.test("arrayExchange -> 1ms*1", function(assert) {
  ...
  BbaseEst.arrayExchange(list1, 0, 1, {
    column: 'sort',
...
QUnit.test("arrayInsert -> 1ms*1", function(assert) {
  ...
  BbaseEst.arrayInsert(list1, 3, 1, {
    column: 'sort',
...
QUnit.test("arrayToObject -> 0ms*1", function(assert) {
  ...
  var object1 = BbaseEst.arrayToObject(list1, 'key', 'value');
...
QUnit.test("arrayFromObject -> 0ms*1", function(assert) {
  ...
  var list1 = BbaseEst.arrayFromObject(object1, 'key', 'value');
  var list2 = [
    { "key": "key1", "value": "value1" },
    { "key": "key2", "value": "value2" }
  ];
...
QUnit.test("hasKey -> 0ms*1", function(assert) {
  ...
  var result = BbaseEst.hasKey(object1, 'name');
  assert.ok(result, 'BbaseEst.hasKey(object1, "name"); => true');
});

/*QUnit.test("BbaseEst.makeMap", function (assert) {
  var map = BbaseEst.makeMap("a,a,aa,a,ah,a");
...
/*QUnit.test("BbaseEst.arrayRemove", function (assert) {
  var list = ['a', 'b', 'b'];
  var result = BbaseEst.arrayRemove(list, 'b');
...
QUnit.test("map -> 1ms*3", function(assert) {
  ...
  var result2 = BbaseEst.map({ 'one': 1, 'two': 2, 'three': 3 }, function(num) {
    return num * 3;
  });
  ...
  var result3 = BbaseEst.map(characters, 'name');
...
QUnit.test("filter -> 2ms*3", function(assert) {
  ...
  var result2 = BbaseEst.filter(list, { "name": "bb", "address": "zjut" });
  ...
  var result3 = BbaseEst.filter(list3, { "album_id": 'Album_00000000000000000000059144' });
...
QUnit.test("findIndex -> 1ms*3", function(assert) {
  ...
  var index3 = BbaseEst.findIndex(list, { name: 'bb', address: 'zjut' });
  assert.equal(index3, 3, 'test multi params of  object : passed!');

  /*var list4 = ["Attach_0000000000000000000667698"];
   var index4 = BbaseEst.findIndex(list4, 'Attach_0000000000000000000667698');
   assert.equal(index4, 0, 'passed');*/
});

QUnit.test('sortBy -> 5ms*4', function(assert) {
  var result = BbaseEst.sortBy([1, 2, 3], function(num) {
    return Math.sin(num);
  });
  ...
  var result4 = BbaseEst.sortBy(characters, ['age', 'name']);
...
QUnit.test('remove -> 2ms*8', function(assert) {
  ...
  var result = BbaseEst.remove(targetList2, 2);
  assert.deepEqual(result, [1, 3, 4, 5], 'remove number');

  var targetList3 = [1, '2', 3, 4, 5];
  var result = BbaseEst.remove(targetList3, '2');
  ...
  var result = BbaseEst.remove(targetList4, { key: '11' });
  ...
  var result = BbaseEst.remove(targetList5, { key: '11', value: '33' });
  ...
  var result = BbaseEst.remove(targetList, removeList, function(targetItem, removeItem) {
